...
function CurrentDateTime() {
    ...
    return (
        <div className="flex items-center gap-2 px-4 py-2 text-sm text-gray-400 border-b border-gray-800/50">
        ...
        <>
            <motion.div
                ...
                className={classNames(
                    ...
                    "bg-[#1E293B] border-r border-bolt-elements-borderColor",
                    "shadow-sm text-sm",
                    isSettingsOpen ? "z-40" : "z-sidebar"
                )}
            >
                <div className="h-12 flex items-center justify-between px-4 border-b border-[#35455e] bg-[#1E293B]">
                    <a href="/" className="font-medium text-sm text-white truncate ml-8 mt-1">
                        Gible
                    </a>
                    <div className="text-white font-medium"></div>
                    <div className="flex items-center gap-3">
                        <span className="font-medium text-sm text-white truncate">{profile?.username || "Guest User"}</span>
                        <div className="flex items-center justify-center w-[32px] h-[32px] overflow-hidden bg-gray-800 text-gray-500 rounded-full shrink-0">
                            {profile?.avatar ? (
                                <img
                                    src={profile.avatar}
                ...
                <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
                    <div className="p-4 space-y-3">
                        <div className="flex gap-2">
                            <a
                                href="/"
                                className="flex-1 flex gap-2 items-center bg-[#225BD6] text-white hover:bg-[#225BD6]/90 rounded-lg px-4 py-2 transition-colors"
                            >
                        ...
                        <div className="relative w-full">
                            <div className="absolute left-3 top-1/2 -translate-y-1/2">
                                <span className="i-ph:magnifying-glass h-4 w-4 text-gray-500" />
                            </div>
                            <input
                                className="w-full bg-[#324152] relative pl-9 pr-3 py-2 rounded-lg focus:outline-none text-sm text-white border border-transparent"
                    ...
                    <div className="flex items-center justify-between text-sm px-4 py-2">
                        <div className="font-medium text-gray-400">Recent Projects</div>
                        {selectionMode && (
                    ...
                    <div className="flex-1 overflow-auto px-3 pb-3">
                        {filteredList.length === 0 && (
                            <div className="px-4 text-gray-400 text-sm">{list.length === 0 ? "No previous conversations" : "No matches found"}</div>
                        )}
                        <DialogRoot open={dialogContent !== null}>
                            {binDates(filteredList).map(({ category, items }) => (
                                <div key={category} className="mt-2 first:mt-0 space-y-1">
                                    <div className="text-xs font-medium text-[#5d6f85] bg-transparent sticky top-0 z-1 bg-white px-4 py-1">
                                        {category}
                                    </div>
                            ...
                            <Dialog onBackdrop={closeDialog} onClose={closeDialog}>
                                ...
                                {dialogContent?.type === "bulkDelete" && (
                                    <>
                                        <div className="p-6 bg-gray-950">
                                            <DialogTitle className="text-white">Delete Selected Chats?</DialogTitle>
                                            <DialogDescription className="mt-2 text-gray-400">
                                                <p>
                                                    You are about to delete {dialogContent.items.length}{" "}
                                                    {dialogContent.items.length === 1 ? "chat" : "chats"}:
                                                </p>
                                                <div className="mt-2 max-h-32 overflow-auto border border-gray-800 rounded-md bg-gray-900 p-2">
                    ...
                    <div className="flex flex-col-reverse items-center justify-center border-t border-[#35455e] px-4 py-3">
                        <SettingsButton onClick={handleSettingsClick} />
                        <ThemeSwitch className="flex items-center justify-center text-[#666] w-full !bg-[#324154] flex items-center justify-center py-1.5 hover:text-bolt-elements-textPrimary hover:bg-bolt-elements-item-backgroundActive/10 transition-colors cursor-pointer mb-2" />
                    </div>
                </div>
            </motion.div>
